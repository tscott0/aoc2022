	...
	for i, l := range lines {
		...
		if len(l) > 0 && strings.Contains(l, "[") {
			...
			for i := 0; i < count; i++ {
				...
				// copy to the end of the destination stack
				stacks[toIdx] = append([]rune{stacks[fromIdx][0]}, stacks[toIdx]...)

				// remove from the source stack
				if len(stacks[fromIdx]) == 1 {
					stacks[fromIdx] = []rune{}
				} else {
					stacks[fromIdx] = stacks[fromIdx][1:]
				}
